...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BurnCoinTEST2\r");
            Send(hSocket, "WHO #BurnCoinTEST2\r");
        } else {
            // randomly join #BurnCoin00-#BurnCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BurnCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BurnCoin%02d\r", channel_number).c_str());
        }
